...
import fdoom.entity.ClientTickable;
import fdoom.entity.Entity;
import fdoom.entity.ItemEntity;
import fdoom.entity.furniture.Chest;
import fdoom.entity.furniture.Crafter;
import fdoom.entity.furniture.DungeonChest;
import fdoom.entity.furniture.Lantern;
import fdoom.entity.furniture.Spawner;
import fdoom.entity.furniture.Tnt;
import fdoom.entity.mob.*;
import fdoom.entity.particle.Particle;
...
public class Level {
	...
	public void updateVisible() {
		...
		while (yy < y1) {
			...
			while (xx < x1) {
				int xd = xx - px;
				int dist = xd * xd + yd;
				if (dist <= viewSize * viewSize) {
					this.visible[xx + yy * this.w] = true;
				}
				++xx;
			}
			++yy;
	...
	public void printEntityStatus(String entityMessage, Entity entity, String... searching) {
		...
		for(String search: searching) {
			try {
				if(Class.forName("fdoom.entity."+search).isAssignableFrom(entity.getClass())) {
	...
	public void trySpawn() {
		...
		for (int i = 0; i < 30 && !spawned; i++) {
			...
			//if ((Updater.getTime() == Updater.Time.Night || depth != 0) && EnemyMob.checkStartPos(this, nx, ny)) {
				...
				if (rnd <= 40) add((new Zombie(lvl)), nx, ny);
				else if (rnd <= 75) add((new Snake(lvl)), nx, ny);
