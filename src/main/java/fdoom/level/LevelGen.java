...
import fdoom.screen.WorldGenDisplay;
...
public class LevelGen {
	...
	private static byte[][] createAndValidateTopMap(int w, int h) {
		...
		do {
			...
			if (count[Tiles.get("Quick Sand").id & 0xff] < 1) continue;
			...
			do {
				...
				for (int i = 0; i < w * h; i++) {
					count[humans[0][i] & 0xfff]++;
				}
				
				//TODO: perform check for graves...
				if (count[Tiles.get("fence").id & 0xff] < 15) continue;
				if (count[Tiles.get("grave stone").id & 0xff] < 9) continue;
	...
	private static byte[][] createAndValidateUndergroundMap(int w, int h, int depth) {
		...
		do {
			...
			//byte[][] result = createAndValidateTopMap(w, h);
	...
	private static byte[][] createTopMap(int w, int h) { // create surface map?
		...
		for (int i = 0; i < w * h / 1200; i++)
		{
			...
			for (int j = 0; j < 45; j++)
			{
				...
				if (xx >= 0 && yy >= 0 && xx < w && yy < h)
				{
					if (map[xx + yy * w] == Tiles.get("grass").id)
					{
						map[xx + yy * w] = Tiles.get("pumpkin").id;
					}
				}
			}
		}
		...
		for (int i = 0; i < w * h / 1200; i++ ) {
			...
			for(int j = 0; j < 12; j++) {
				...
				if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
					if (map[xx + yy * w] == Tiles.get("grass").id) {
						map[xx + yy * w] = Tiles.get("grave stone").id;
					}
				}
			}
		}
			...
			for (int i = 0; i < w * h / 1200; i++) {
				...
				for (int j = 0; j < 200; j++) {
					...
					if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
						if (map[xx + yy * w] == Tiles.get("grass").id) {
							map[xx + yy * w] = Tiles.get("tree").id;
						} else if (map[xx + yy * w] == Tiles.get("snow").id) {
							map[xx + yy * w] = Tiles.get("snow tree").id;
						}
					}
				}
			}
		...
		for (int i = 0; i < w * h / 100; i++) {
			...
			if (xx >= 0 && yy >= 0 && xx < w && yy < h) {
				if (map[xx + yy * w] == Tiles.get("sand").id) {
					map[xx + yy * w] = Tiles.get("Quick Sand").id;
					break;
				}
			}
		}
		...
		//map[random.nextInt(w) + random.nextInt(h) * w] = Tiles.get("Quick Sand").id;
			...
			for (int i = 0; i < w * h / 380; i++) {
				for (int j = 0; j < 10; j++) {
					if (xx < w - r && yy < h - r) {
						...
						//map[xx + 2 + (yy + 2) * w] = (byte) ((Tiles.get("Quick Sand").id & 0xff));
	...
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println(LevelGen.worldSeed);
			int w = 128;
			int h = 128;
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					if (map[i] == Tiles.get("Quick Sand").id) pixels[i] = 0xff0000;
					if (map[i] == Tiles.get("Stairs Up").id) pixels[i] = 0xffffff;
					if (map[i] == Tiles.get("Cloud Cactus").id) pixels[i] = 0xff00ff;
					if (map[i] == Tiles.get("snow").id) pixels[i] = 0x800080;
					if (map[i] == Tiles.get("tall grass").id) pixels[i] = 0xff0000;
					if (map[i] == Tiles.get("pumpkin").id) pixels[i] = 0xffa500;
					if (map[i] == Tiles.get("grave stone").id) pixels[i] = 0x42d1f4;
					if (map[i] == Tiles.get(33).id) pixels[i] = 0x41F4BE;
				}
			}
