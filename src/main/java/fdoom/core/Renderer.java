...
import fdoom.entity.furniture.Bed;
import fdoom.entity.mob.Player;
import fdoom.gfx.*;
import fdoom.gfx.Ellipsis.DotUpdater.TickUpdater;
import fdoom.gfx.Ellipsis.SmoothEllipsis;
...
import fdoom.screen.Menu;
import fdoom.screen.RelPos;
...
public class Renderer extends Game {
	...
	public static void render() {
		...
		if (!canvas.hasFocus() && !ISONLINE && !Game.continous) renderFocusNagger(); // calls the renderFocusNagger() method, which creates the "Click to Focus" message.
	...
	private static void renderGui() {
		 ...
		 */
		Font.renderFrame(screen, "", 26, 0, 35, 2,
				Font.DEFAULT_BACKGROUND_COLOR, Font.DEFAULT_BORDER_COLOR,
				Font.DEFAULT_TITLE_COLOR);
		...
		if (isMode("creative") || ac >= 10000)
			Font.draw("	x" + "^", screen, WIDTH - 70, 8, Color.get(-1, 333, 444, 555));
		else
			Font.draw("	x" + ac, screen, WIDTH - 70, 8, Color.get(-1, 555));
		//displays arrow icon
		screen.render(WIDTH - 72, 7, 13 + 5 * 32, Color.get(-1, 111, 222, 430), 0);
		...
		// TODO: Need to move this to the top left.
		...
		if (player.activeItem instanceof ToolItem) {
			...
			Font.draw(dura + "%", screen, WIDTH - 38, 8, Color.get(-1, 30));
		}
		...
		if (!isMode("creative")) {
			...
			// RIGHT EDGE OF SCREEN IS: (Screen.w - 80)

			/**
			 * This is the Health box.
			 */
			Font.renderFrame(screen, "", 0, 0, 10, 4,
					Font.DEFAULT_BACKGROUND_COLOR, Font.DEFAULT_BORDER_COLOR,
					Font.DEFAULT_TITLE_COLOR);
			/**
			 * This is the Selected item box.
			 */

			Font.renderFrame(screen, "", 11, 0, 25, 2,
					Font.DEFAULT_BACKGROUND_COLOR, Font.DEFAULT_BORDER_COLOR,
					Font.DEFAULT_TITLE_COLOR);
			...
			for (int i = 0; i < Player.maxStat; i++) {
				...
				screen.render(i * 8 + 4, 4, 0 + 12 * 32, color, 0);
				...
				if (player.staminaRechargeDelay > 0) {
					...
					screen.render(i * 8 + 4, 8 + 5, 1 + 12 * 32, color, 0);
				} else {
				...
				screen.render(i * 8 + 4, 8 + 5 + 8, 2 + 12 * 32, color, 0);
			}
		}
		
		/// CURRENT ITEM
		if (player.activeItem != null) // shows active item sprite and name in bottom toolbar, if one exists.
			player.activeItem.renderInventory(screen, 12 * 7 + 10, 8, false);
	}
	...
	private static void renderFocusNagger() {
		String msg = "Come Back!"; // the message when you click off the screen.
		...
		int h = 1;
		int txtcolor = Color.get(-1, 1, Color.hex("#2c2c2c"), 445);
