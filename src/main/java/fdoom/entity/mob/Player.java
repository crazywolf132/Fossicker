...
import fdoom.entity.ItemHolder;
import fdoom.entity.furniture.Bed;
import fdoom.entity.furniture.DeathChest;
...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	public void tick() {
		...
		if (onTile == Tiles.get("Stairs Down") || onTile == Tiles.get("Stairs Up") || onTile == Tiles.get("Quick Sand")) {
		...
		if (Game.getMenu() == null && !Bed.inBed(this)) {
			...
			if(input.getKey("map").clicked) {
				Game.setMenu(new MapMenu());
			}

			if (input.getKey("inventory").clicked && !use()) // !use() = no furniture in front of the player; this prevents player inventory from opening (will open furniture inventory instead)
