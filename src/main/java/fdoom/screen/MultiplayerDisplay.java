...
public class MultiplayerDisplay extends Display {
	...
	private static String domain = "http://localhost:4000/";
	private static String apiDomain = domain + "/users/";
		...
		Unirest.get(domain).asBinaryAsync(new Callback<InputStream>() {
			...
			public void completed(HttpResponse<InputStream> httpResponse) {
				if(httpResponse.getStatus() == 200)
					...
					System.err.println("warning: fdoom site ping returned status code " + httpResponse.getStatus());
			...
			public void cancelled() {
				...
				if(savedUsername.length() == 0 || savedUUID.length() == 0) {
					...
					setError("could not connect to fossickers Doom account server, but no login data saved; cannot enter offline mode.", false);
					return;
				}
	...
	private void login(String email, String password) {
		...
		Unirest.post(apiDomain+"authenticate")
	...
	private void fetchName(String uuid) {
		...
		try {
			response = Unirest.post(apiDomain+"fetch")
		...
		if(response != null) {
			...
			switch(json.getString("status")) {
				...
				case "success":
					if(Game.debug) System.out.println("successfully received username from fossickers doom server");
